...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    fprintf(stderr,
            "Usage: %s number_of_clients number_of_samples sampling_period "
            "processing_delay\n",
            argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  int fout =
      open("server.txt", O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
  assert(fout >= 0);
  ...
  for (i = 0; i < nsmp; i++) {
    ...
    if (udelsmp > last_processing_time) {
        usleep(udelsmp - last_processing_time);
    }
    ...
    // Now we only detect possible error, but we should also check the number of
    // written bytes and repeat writing if only part of the line was written?
